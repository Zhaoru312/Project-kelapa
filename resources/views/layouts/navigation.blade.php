<nav x-data="{ open: false }">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="hidden sm:-my-px sm:flex">
                    <x-nav-link :href="route('homepage.belanja')" :active="request()->routeIs('homepage.belanja')">
                        {{ __('Belanja') }}
                    </x-nav-link>
                </div>
                @if(Auth::check() && Auth::user()->usertype == 1)
                    <div class="hidden sm:-my-px sm:flex">
                        <x-nav-link :href="route('homepage.dashboard')" :active="request()->routeIs('homepage.dashboard')">
                            {{ __('Dashboard') }}
                        </x-nav-link>
    ...
    <div :class="{ 'block': open, 'hidden': ! open }" class="hidden sm:hidden">>
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('homepage.belanja')" :active="request()->routeIs('homepage.belanja')">
                {{ __('Belanja') }}
            </x-responsive-nav-link>

            @if(Auth::check() && Auth::user()->usertype == 1)
                <x-responsive-nav-link :href="route('homepage.dashboard')" :active="request()->routeIs('homepage.dashboard')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
